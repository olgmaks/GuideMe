...
nav {
	...
	<script type="text/javascript">
		$(document)
				.ready(
						function() {
							$('#ratingEvent').ratings(5, '${mark}').bind(
									'ratingchanged',
									function(event, data) {
										rate(data.rating)
										$('#example-rating-1')
	...
	<fmt:bundle basename="locale.event.messages">
					...
					<div id="w">
						<div id="content" class="clearfix">
								...
								<c:if test="${showJoin}">
									<form action="joinEvent.do" method="POST">
										...
										<button class="btn light-blue waves-effect waves-light"
											...
											<fmt:message key="event.JoinEvent" />
											<i class="mdi-content-add-circle-outline right"></i>
										</button>


										<br> <br>
										<fmt:message key="event.Status" />
										<select class="browser-default" id="status" name="status"
											style="width: 50%; text-align: left; font-size: 100%; text-transform: capitalize">
											<option selected value="guest"><fmt:message
													key="event.Guest" /></option>
											<option value="resident"><fmt:message
													key="event.Resident" /></option>
										</select>
										...
										<br>
										<fmt:message key="event.Apartments" />
										: <select class="browser-default" id="bedCountSelect"
											name="bedCountSelect"
											style="width: 50%; margin-top: 10px; text-align: left; font-size: 100%; text-transform: capitalize">
											<option value="need"><fmt:message
													key="event.NeedApartments" /></option>
											<option value="accept"><fmt:message
													key="event.AcceptingGuests" /></option>
										</select> <br>
										<fmt:message key="event.Persons" />
										: <select class="browser-default" id="bedCount"
											name="bedCount"
											style="width: 50%; margin-top: 10px; text-align: left; font-size: 100%; text-transform: capitalize">
									...
									<form action="joinEvent.do" method="POST">
										...
										<button class="btn light-blue waves-effect waves-light"
											...
											<c:if test="${showCancel}">
												<fmt:message key="event.CancelRequest" />
											</c:if>
											<c:if test="${showQuit}">
												<fmt:message key="event.QuitEvent" />
											</c:if>
							...
							<c:if test="${!isModerator}">
								...
								<br>
								<br>
								<b> <fmt:message key="event.EventStatus" />:
									${event.status}
								</b>
								<br>
							...
							<c:if test="${isModerator}">
								<form action="changeEventStatus.do" method="POST"
									...
									<br>
									<fmt:message key="event.EventStatus" />
									: <select class="browser-default" id="eventStatus"
										name="eventStatus"
										style="width: 50%; text-align: left; font-size: 100%; text-transform: capitalize">

										<option ${selActive} value="active"><fmt:message
												key="event.Active" /></option>
										<option ${selFilled} value="filled"><fmt:message
												key="event.Filled" /></option>
										<option ${selCancelled} value="cancelled"><fmt:message
												key="event.Canceled" /></option>
										<option ${selDone} value="done"><fmt:message
												key="event.Done" /></option>
									</select>

									<button class="btn light-blue waves-effect waves-light"
										type="submit" name="action"
										style="width: 50%; margin-top: 10px; text-align: left; font-size: 100%; text-transform: capitalize">
										<fmt:message key="event.Change" />
										</i>
									</button>
								...
								<form method="POST" id="tagform-editonly">
									<input type="hidden" name="id" value="${requestScope.event.id}">
									<br>
									<fmt:message key="event.Interests" />
							...
							<ul class="collection">
								<li class="collection-item avatar"><img
									...
									<span class="title"><a
										href="userProfile.do?id=${event.moderator.id}"><fmt:message
												key="event.Author" />: ${event.moderator.firstName}
											${event.moderator.lastName}</a></span>
							...
							<section id="bio">
								...
								<p>
									<span><b><fmt:message key="event.Description" />: </b>
									</span>
								...
								<c:if test="${isMember}">
									...
									<c:if test="${!isModerator}">
										<p>
											<span><b> <fmt:message key="event.YourMark" />
											</b></span>
										</p>
									</c:if>
										...
										<c:when test="${not empty userLogined}">
											...
											<form action="adminEventRequest.do?action=commentEvent"
												...
												<button class="btn light-blue waves-effect waves-light"
													type="submit" name="action"
													style="width: 50%; margin-top: 10px; text-align: left; font-size: 100%; text-transform: capitalize">
													<fmt:message key="event.AddComment" />
													<i class="mdi-content-add-circle-outline right"></i>
												</button>
											</form>
							...
							<section id="chat" class="hidden">
								...
								<div class="col s12" style="margin-top: 10px;">
									...
									<input type=text id="enterMessage"
										...
										type="submit" onclick="sendMessage();" id="submitButton"
										value="${SendSubmitName}">
								</div>
							</section>

							<section id="members" class="hidden">
								<p>
									<fmt:message key="event.Members" />
									:
								</p>
									...
									<c:forEach items="${requestScope.members}" var="m">
										...
										<form id="userFriendFormWithId${m.user.id}">
											...
											<li class="collection-item avatar"
												...
												</p> <a class="secondary-content"><i class="material-icons"><fmt:message
															key="${m.status}" /> </i></a></li>
										</form>
									</c:forEach>
								</ul>
							...
							<section id="edit" class="hidden">
								<p>
									<fmt:message key="event.EditEvent" />
									:
								</p>
										...
										<div class="row">
											<div class="input-field col s6">
												...
												<input type="text" name="name" id="name"
													...
													for="name"> <fmt:message key="editEvent.eventName" />:
												</label>
												...
												<table style="width: 100%;">
													<tr>
														...
														<td style="width: 34%;"><label for="dateFrom">
																<fmt:message key="event.From" />:
														</label> <input type="date" class="datepicker" id="dateFrom"
															name="dateFrom" value="${dateFrom}" /> <span
															id="dateMessage"></span></td>

														<td style="width: 8%;"><label for="hourFrom">
																<fmt:message key="event.Hour" />:
														</label> <input type="number" name="hourFrom" min="00" max="23"
															id="hourFrom" value="${hourFrom}"></td>
														<td style="width: 0%;">:</td>

														<td style="width: 8%;"><label for="minuteFrom">
																<fmt:message key="event.Minute" />:
														</label> <input type="number" name="minuteFrom" min="00" max="59"
															id="minuteFrom" value="${minuteFrom}"></td>

														<td style="width: 34%;"><label for="dateTo">
																<fmt:message key="event.To" />
														</label> <input type="date" class="datepicker" id="dateTo"
															name="dateTo" value="${dateTo}" /> <span
															id="dateMessage"></span></td>

														<td style="width: 8%;"><label for="hourTo"> <fmt:message
																	key="event.Hour" />:
														</label> <input type="number" name="hourTo" min="00" max="23"
															id="hourTo" value="${hourTo}"></td>
														<td style="width: 0%;">:</td>

														<td style="width: 8%;"><label for="minuteTo">
																<fmt:message key="event.Minute" />:
														</label> <input type="number" name="minuteTo" min="00" max="59"
															id="minuteTo" value="${minuteTo}"></td>
										...
										<div class="row">
                                            ...
                                            <table style="width: 100%;">
														<tr>
															...
															<td style="width: 30%;"><label><fmt:message
																		key="event.Country" /></label> <select
															...
															<td style="width: 30%;"><label> <fmt:message
																		key="event.City" />
															</label> <select id="cityByLang_${lang.id}"
																class="browser-default">
																	<option selected value="choose">
																		<fmt:message key="loginpage.city.choosecountryfirst" />
																	</option>
															</select> <script>
																$(
																		'#cityByLang_${lang.id}')
										...
										<div class="row">
											<div class="input-field col s12">
												...
												<label for="description"><fmt:message
														key="editEvent.Description" />:</label>
											</div>
										</div>
																...
																<table style="width: 100%;">
																	<tr>
																		...
																		<td style="width: 25%;"><input type="number"
																			...
																			<label for="partisipant_limit"> <fmt:message
																					key="event.Partisipantlimit" />
																		</label></td>
                                    ...
                                    Status: <select
												...
												<c:if test="${getstatus=='guest'}">
													<option selected="" value="guest">
														<fmt:message key="event.Guest" /></option>
													<option value="resident"><fmt:message
															key="event.Resident" />
													</option>
												</c:if>
												<c:if test="${getstatus=='resident'}">
													<option value="guest"><fmt:message
															key="event.Guest" /></option>
													<option selected="" value="resident"><fmt:message
															key="event.Resident" /></option>
												</c:if>
											...
											<br>
											<fmt:message key="event.Apartments" />: <select
												class="browser-default" id="bedCountSelect"
												name="bedCountSelect"
												style="width: 50%; margin-top: 10px; text-align: left; font-size: 100%; text-transform: capitalize">
												<c:if test="${apartments=='need'}">
													<option value="need" selected><fmt:message
															key="event.NeedApartments" />:
													</option>
													<option value="accept"><fmt:message
															key="event.AcceptingGuests" />:
													</option>
												</c:if>
												<c:if test="${apartments=='accept'}">
													...
													<option value="accept" selected><fmt:message
															key="event.AcceptingGuests" />:
													</option>
												</c:if>
											</select>
											<fmt:message key="event.Persons" />: <br>
										...
										<button class="btn light-blue waves-effect waves-light"
											...
											<fmt:message key="EditEvent.Edit" />
										</button>
									</form>
								</div>
							</section>
							...
							<c:if test="${isModerator}">
								<section id="requests" class="hidden">
									<p>
										<fmt:message key="event.Requests" />
										:
									</p>
										...
										<c:forEach items="${requestScope.requests}" var="m">
											...
											<form id="userFriendFormWithId${m.user.id}">
												...
												<li class="collection-item avatar"
													...
													<p>
														<c:if test="${m.bedCount > 0}">
															<fmt:message key="js.CanAccept" />: ${m.bedCount} <fmt:message
																key="js.persons" />
														</c:if>
														<c:if test="${m.bedCount < 0}">
															<fmt:message key="js.NeedLodjingFor" />: ${-m.bedCount} <fmt:message
																key="js.persons" />
														</c:if>
													...
													</p> <a class="secondary-content"><i class="material-icons">
															<fmt:message key="${m.status}" />
													</i></a></li>
											</form>
										</c:forEach>
		...
		<div id="confirmDeleteModal" class="modal" style="width: 25%">
			<div class="modal-content">
				<h4>
					<fmt:message key="event.DeletePhoto" />
				</h4>

				<p>
					<fmt:message key="event.AreYouSure" />
					?
				</p>
			</div>
			<div class="modal-footer">
				<a href="#_" id="modalDeletePhotoAnchor" data-value=""
					class=" modal-action modal-close waves-effect waves-green btn-flat"><fmt:message
