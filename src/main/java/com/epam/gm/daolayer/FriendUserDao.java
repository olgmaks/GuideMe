...
public class FriendUserDao extends AbstractDao<FriendUser> {
    ...
    //gryn
    private static final String  GET_BY_USER_AND_FRIEND =  
    		"fu WHERE fu.user_id = %S AND fu.friend_id = %S";
...
//            "AND (u.first_name RLIKE '.*%2$s.*' OR u.last_name RLIKE '.*%2$s.*' OR c.name RLIKE '.*%2$s.*'));";
    ...
    public void sendFriendRequest(int userId, int friendId) throws SQLException, IllegalAccessException {
        //gryn - if exists
        if(!getByUserAndFriend(userId, friendId).isEmpty()) {
        	return;
        }
    ...
    public void sendFriendRequest(FriendUser friendUser) throws SQLException, IllegalAccessException {
        //gryn - if exists
        if(!getByUserAndFriend(friendUser.getUserId(), friendUser.getFriendId()).isEmpty()) {
        	return;
        }
    ...
    public void acceptFriendRequest(FriendUser friendUser) throws SQLException, IllegalAccessException {
        ...
        //gryn - if exists
        if(!getByUserAndFriend(friendId, userId).isEmpty()) {
        	return;
        }
    ...
    //gryn
    public List<FriendUser> getByUserAndFriend(Integer userId, Integer friendId) throws SQLException {
    	System.out.println("QUERY: ");
    	System.out.println(String.format(GET_BY_USER_AND_FRIEND, userId, userId));
    	
        return super.getWithCustomQuery(String.format(GET_BY_USER_AND_FRIEND, userId, friendId));
    }
