...
public class UserInEventDao extends AbstractDao<UserInEvent> {
	...
	private static final String GET_BY_EVENT_ONLY_REQUESTERS =
			" uie WHERE uie.is_member = FALSE AND uie.event_id = ? ";
	
	private static final String DELETE_BY_EVENT_AND_USER = 
			" event_id = ?eventId AND user_id = ?userId";
	
	private static final String GET_EVENTS_BY_USER_ID_WHERE_USER_NOT_MODERATOR = "uie JOIN event e WHERE uie.event_id=e.id AND NOT uie.user_id=e.moderator_id AND uie.user_id=%s;";
	...
	public List<UserInEvent> getAllUserInEventWhereUserNotModeratorByUserId(
			int id) throws SQLException {
		List<UserInEvent> list = super.getWithCustomQuery(String.format(
				GET_EVENTS_BY_USER_ID_WHERE_USER_NOT_MODERATOR, id));
    ...
    public List<UserInEvent> getEventsByUserId(int userId) throws SQLException {
     ...
     public List<UserInEvent> getByEventOnlyRequesters(Integer eventId) throws SQLException {
         return getWithCustomQuery(GET_BY_EVENT_ONLY_REQUESTERS.replace("?", eventId.toString()));
     }
